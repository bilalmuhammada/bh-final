        ...
        <div class="topbar-wrapper">
            ...
            <div class="topbar-items-group">
                ...
                <div class="selection-item">
                    <select class="form-control country_dropdown1 country_dropdown" name="country_dropdown"
                        style="width:160px;" id="country_dropdown">
                        <option value="">All Countries</option>
                        @foreach($countries as $country)
                            <option {{ $country->id == request()->country ? 'selected' : '' }}
                                data-flag-url="{{ $country->image_url }}" value="{{ $country->id }}">
                                {{ $country->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="selection-item">
                    <select class="form-control currency_dropdown" name="currency_dropdown" style="width:100px;">
                        ...
                        @foreach($currency as $currencyn)
                            <option data-currency-id="{{ $currencyn->currency }}" {{ $currencyn->currency_short_name == session('app_currency', 'USD') ? 'selected' : '' }}
                                data-flag-url="{{ $currencyn->image_url }}" value="{{ $currencyn->currency_short_name }}">
                ...
                @if (session()->has('user'))
                    ...
                    <div class="dropdown" style="margin-left: auto;">
                        <a href="{{ env('BASE_URL') . 'notifications' }}"
                            ...
                            @if(count($notifications) > 0)
                                <span
                                    class="badge-premium-green">{{ count($notifications) > 99 ? '99' : count($notifications) }}</span>
                            @endif
                        </a>
                        <div class="dropdown-menu" style="width:420px;">
                            @if (count($notifications) > 0)
                                <div class="px-2 py-1 border-bottom d-flex justify-content-between align-items-center">
                                    ...
                                    <span
                                        class="badge-new-green">{{ count($notifications) > 99 ? '99' : count($notifications) }}
                                        New</span>
                                </div>
                                        ...
                                        <div class=" list-group-item-action border-bottom pl-2 pr-2 topbar-dropdown-row">
                                            <div class="d-flex align-items-center position-relative">
                                                ...
                                                <div class="ms-3 flex-grow-1 px-2">
                                                    <h6 class="mb-0  fw-bold ad-item-title" style="font-size: 13px;">
                                                        {{$notification->title}}</h6>
                                                    <p class="mb-0 text-muted small text-truncate" style="max-width: 180px;">
                                                        {{$notification->message}}</p>
                                                    <small class="text-muted"
                                                        style="font-size: 10px;">{{ \Carbon\Carbon::parse($notification->created_at)->diffForHumans() }}</small>
                                                </div>
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu-custom">
                                                        ...
                                                        <a href="#" style="color: red !important;"
                                                            onclick="removeNotification(this)">Remove</a>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="text-center pt-1 pb-1 border-top">
                                    <a href="{{ env('BASE_URL') . 'notifications'}}" class="fw-bold color-logo"
                                        style="font-size: 14px;">View all Notifications</a>
                                </div>
                            @else
                    ...
                    <div class="dropdown">
                        <a href="{{ env('BASE_URL') . 'my-favourites' }}"
                            ...
                            @if($favourite_ads_count > 0)
                                <span
                                    class="badge-premium-green">{{ $favourite_ads_count > 99 ? '99' : $favourite_ads_count }}</span>
                            @endif
                        </a>
                        <div class="dropdown-menu" style="width:380px;">
                            @if (count($favourite_ads) > 0)
                                <div class="px-2 py-1 border-bottom d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge-new-green">{{ $favourite_ads_count > 99 ? '99' : $favourite_ads_count }}
                                        New</span>
                                </div>
                                <div class="list-group list-group-flush">
                                    @foreach($favourite_ads as $favourite_ad)
                                        <a href="{{ env('BASE_URL') . 'ads/detail/' . $favourite_ad->id . '?country=' . request()->country . '&city=' . request()->city }}"
                                            ...
                                            <div class="ms-3 flex-grow-1 px-2">
                                                <h6 class="mb-2 text-truncate fw-bold ad-item-title" style="font-size: 14px;">
                                                    {{ $favourite_ad->title }}</h6>
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <small
                                                        class="text-muted">{{ $favourite_ad->category->name ?? 'General' }}</small>
                                                    <h6 class="mb-0 fw-bold price-text" style="font-size: 12px;">
                                                        {{ \App\Helpers\SiteHelper::priceFormatter($favourite_ad->price, session('app_currency', 'USD')) }}
                                                    </h6>
                                                </div>
                                            </div>
                                        </a>
                                ...
                                <div class="text-center pt-1 pb-1 border-top">
                                    <a href="{{ env('BASE_URL') . 'ads?type=favourite' }}" class="fw-bold  color-logo"
                                        style="font-size: 14px;">View all Favorites</a>
                                </div>
                            @else
                        ...
                        @if(count($chats) > 0)
                            <div class="dropdown-menu shadow-lg" style="width:420px;">
                                    ...
                                    @foreach($chats->take(3) as $message)
                                        <a href="{{ route('chats') . '/detail/' . $message->id . '?country=' . request()->country . '&city=' . request()->city }}"
                                            ...
                                            <div class="ms-3 flex-grow-1 px-2 overflow-hidden">
                                                <h6 class="mb-1 fw-bold text-truncate ad-item-title" style="font-size: 14px;">
                                                    {{ Str::limit($message->chat->ad->title ?? 'Untitled Ad', 30) }}</h6>
                                            ...
                                            <img src="{{ $message->chat->ad->main_image_url ?? 'https://placehold.co/45' }}"
                                ...
                                <div class="pt-1 pb-1 text-center border-top">
                                    <a href="{{ route('chats') . '?country=' . request()->country . '&city=' . request()->city }}"
                                        class="fw-bold color-logo" style="font-size: 14px;">View all Chats</a>
                                </div>
                            </div>
                        @endif
                    </div>
                    ...
                    <a href="{{ env('BASE_URL') . 'user/ads?country=' . request()->get('country') . '&city=' . request()->get('city') }}"
                        class="topbar-dropdown-trigger white-space-nowrap" style="white-space: nowrap;">
                        My Ads
                    </a>
                    ...
                    <div style="display: flex; align-items: center;">
                        ...
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu dropdown-menu-right p-0"
                                style="min-width:70px; border: 1px solid #eee !important; margin-top: 2px !important; left: 20px !important; right: auto !important;">
                                <div class="list-group list-group-flush">
